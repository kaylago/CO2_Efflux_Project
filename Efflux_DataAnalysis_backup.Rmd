...
setwd("C:/Users/kkmgo/Dropbox/CO2_Efflux/")
getwd()
detach(package:plyr)
...
counts <-c(1868,3179,2125,4069,4837,3775)
...
hatch14$success <- as.numeric(hatch14$success)

#hatch14 <- hatch14 %>% drop_na()

hatch14 <- hatch14[!is.na(hatch14$laid),]
...
hatch14%>% 
  ...
  summarise(count = sum(laid))

#rish_13<- hatch14 %>% filter(location=="Rish") %>% filter(year==2013) 

#rish_13 %>% summarise(sum.laid=sum(laid),sum.emerged=sum(emerged))



#write.csv(rish_13,"C:/users/kkmgo/Dropbox/rish_13.csv")

yearly_hatch <- hatch14 %>% group_by(location,year,zone) %>% summarize(total=sum(laid),emerged=sum(emerged),mean.success=mean(success)) 
yearly_hatch <- yearly_hatch %>% mutate(total.success=emerged/total*100)
yearly_hatch <- yearly_hatch%>% mutate(failed=total-emerged)


mean_efflux <- total.data.means %>% mutate(location=ifelse(location=="SJP-Rish","Rish",as.character(location)))

tre_efflux <- total.data.tre %>% mutate(location=ifelse(location=="SJP-Rish","Rish",as.character(location)))

yearly_hatch <- merge(yearly_hatch,mean_efflux,by=c("location","zone"))
yearly_hatch <- merge(yearly_hatch,tre_efflux,by=c("location","zone"))
...
detach(package:plyr)
library(dplyr)
yearly_hatch %>% group_by(location,year) %>% summarise(emerged=sum(emerged),failed=sum(failed))

zone_bybeach_byyear<- yearly_hatch %>% group_by(location,zone,year,mean.eff,tre.zone,upper,lower) %>% summarize(total=sum(total),emerged=sum(emerged),failed=sum(failed))

zone_matrix <- matrix(c(805,1010,1467,3139,6558,6874),nrow=2,ncol=3)
chisq.test(zone_matrix)


zone_mat_2011 <- matrix(c(489,606,80,568,326,786),nrow = 2,ncol = 3)
chisq.test(zone_mat_2011)

zone_mat_2012 <- matrix(c(47,263,1208,2098),nrow = 2,ncol = 2)
chisq.test(zone_mat_2012)

zone_mat_2013 <- matrix(c(119,48,602,863,2290,1744),nrow = 2,ncol = 3)
chisq.test(zone_mat_2013)

zone_mat_2014 <- matrix(c(150,93,785,1708,2734,2257),nrow = 2,ncol = 3)
chisq.test(zone_mat_2014)

beach_mat_2011 <- matrix(c(15,662,661,368,219,930),nrow = 2,ncol = 3)
chisq.test(beach_mat_2011)

beach_mat_2012 <- matrix(c(0,674,996,976,259,711),nrow = 2,ncol = 3)
chisq.test(beach_mat_2012)

beach_mat_2013 <- matrix(c(480,929,1668,588,863,1127),nrow = 2,ncol = 3)
chisq.test(beach_mat_2013)

##zone
litt_berm <- matrix(c(805,1010,1467,3139),nrow=2,ncol=2)
chisq.test(litt_berm)


litt_dune <- matrix(c(805,1010,6558,6874),nrow=2,ncol=2)
chisq.test(litt_dune)

berm_dune <- matrix(c(1467,3139,6558,6874),nrow=2,ncol=2)
chisq.test(berm_dune)

yearly_sums <- yearly_hatch %>% group_by(location,year) %>% summarize(total=sum(total),emerged=sum(emerged)) %>% mutate(total.success=emerged/total*100)

zone_sums<- yearly_hatch %>% group_by(zone) %>% summarize(total=sum(total),emerged=sum(emerged)) %>% mutate(total.success=emerged/total*100)

yearly_hatch %>% group_by(location) %>% summarize(total=sum(total),emerged=sum(emerged)) %>% mutate(total.success=emerged/total*100)

yearly_hatch%>% 
  group_by(zone) %>% 
  summarise(record_count = n())

#rish_hatch <- yearly_hatch %>% filter(location=="Rish")
#rish_hatch %>% group_by(year) %>% summarise(count=sum(total))
#rish_hatch %>% group_by(year) %>% summarise(count=sum(emerged))


library(nlme)

hatchlm <- lm(total.success~mean.eff*zone*location,data=yearly_hatch)
summary(hatchlm)
anova(hatchlm)
emmeans(hatchlm, list(pairwise ~ location), adjust = "tukey")
...
detach(package:dplyr)
library(plyr)
yearly_hatch <- yearly_hatch %>% mutate(success.rounded=round_any(total.success,1))

hatchglm <- glm(success.rounded~mean.eff+zone*location,family = "poisson",data=yearly_hatch)
summary(hatchglm)
anova(hatchglm)
emmeans(hatchglm, list(pairwise ~ location), adjust = "tukey")


library(tidyr)
library(dplyr)

#table(hatchdata$survivorship)

#print(table)

obsfreq <- matrix(c(1868,2125,4837,3179,4069,3775),nrow=3,ncol=2)

colnames(obsfreq)= c("hatched","failed")
rownames(obsfreq)=c("EAFB","Salinas","Rish")

print(obsfreq)

chisq.test(obsfreq) #sig x^2=849.761 and df=2 and p-val < 2.2 e-16
...
obs_sal_eafb <- matrix(c(2125,4069,1868,3179), nrow=2,ncol=2)
print(obs_sal_eafb)

chisq.test(obs_sal_eafb)#not sig

obs_sal_rish <- matrix(c(2125,4069,4837,3775),nrow=2,ncol=2)

chisq.test(obs_sal_rish)#sig

obs_rish_eafb <- matrix(c(4837,3775,1868,3179),nrow=2,ncol=2)
...
tiff("C:/Users/kkmgo/Dropbox/CO2_Efflux/Figures/figure9_hatchdata.tiff",res=600,width=8,height=6,units="in")
grid::grid.draw(g)
dev.off()
...
means.berm.2012 <- mean_efflux %>% filter(zone=="berm")

means.berm.2012 <- means.berm.2012%>% mutate(total=rep(0),emerged=rep(0),success.rounded=rep(0),mean.success=rep(0),failed=rep(0),total.success=rep(NA),year=rep(2012))

means.berm.2012 <- merge(means.berm.2012,tre_efflux)

means.lit.2013 <- mean_efflux %>% filter(zone=="littoral") %>% mutate(total=rep(0),emerged=rep(0),success.rounded=rep(0),mean.success=rep(0),failed=rep(0),total.success=rep(NA),year=rep(2013))%>% filter(location != "Salinas")

means.lit.2013<-merge(means.lit.2013,tre_efflux)

means.lit.2014 <- mean_efflux %>% filter(zone=="littoral") %>% mutate(total=rep(0),emerged=rep(0),success.rounded=rep(0),mean.success=rep(0),failed=rep(0),total.success=rep(NA),year=rep(2014)) %>% filter(location != "Rish")

means.lit.2014<-merge(means.lit.2014,tre_efflux)

yearly_hatch <- rbind(yearly_hatch,means.lit.2014,means.berm.2012,means.lit.2013)

yearly_hatch <- yearly_hatch %>% arrange(location,zone,year)

yearly_hatch <- yearly_hatch %>% mutate(lower=ifelse(lower<0,0,lower))

zone_bybeach_byyear <- zone_bybeach_byyear %>% mutate(total.success=emerged/total*100) %>% mutate(total.success=ifelse(total.success=="NaN",0,total.success))
  
yearly_mean_succ <- zone_bybeach_byyear %>% group_by(location,zone,mean.eff,tre.zone,upper,lower)%>%summarize(mean.success=mean(total.success))

#hatch14_sums <- merge(hatch14_sums,total.data.tre.alt,by=c("location","zone"))

yearly_hatch$zone <- factor(yearly_hatch$zone, levels=c("littoral","berm","dunes"))

yearly_hatch$location <- factor(yearly_hatch$location, levels=c("EAFB","Rish","Salinas"),labels = c("EAFB","Rish","Salinas"))

zone_bybeach_byyear$zone <- factor(zone_bybeach_byyear$zone, levels=c("littoral","berm","dunes"))

zone_bybeach_byyear$location <- factor(zone_bybeach_byyear$location, levels=c("EAFB","Rish","Salinas"),labels = c("EAFB","Rish","Salinas"))


hatch.plot2 <- ggplot(subset(zone_bybeach_byyear,year==2014),aes(x=zone,y=total.success,fill=zone))+
...
tiff("C:/Users/kkmgo/Dropbox/CO2_Efflux/Figures/figure10_2014hatchdata.tiff",res=600,width=8,height=6,units="in")
grid::grid.draw(g)
dev.off()
#grid::grid.draw(g)
...
yearly_hatch2 <- yearly_hatch %>% group_by(location,zone,mean.eff,tre.zone,upper,lower) %>% summarize(total=sum(total),emerged=sum(emerged))

yearly_hatch2 <- yearly_hatch2 %>% mutate(total.success=emerged/total*100) %>% mutate(total.success=ifelse(total.success=="NaN",0,total.success))

hatch.plot3 <- ggplot(yearly_hatch2,aes(x=zone,y=total.success,fill=zone))+
  geom_col(stat = "identity")+
  geom_point(aes(x=zone,y=mean.eff*20),shape=15,size=3,color="midnightblue")+
  geom_errorbar(aes(ymin=lower*20,ymax=upper*20))+
  geom_point(aes(y=tre.zone*20),color="grey23",shape=15,size=3)+
...
g <- ggplot_gtable(ggplot_build(hatch.plot3))
striprt <- which(grepl('strip-t', g$layout$name))
fills <- c("steelblue3","coral","darkolivegreen4")
k <- 1
for (i in striprt) {
...
tiff("C:/Users/kkmgo/Dropbox/CO2_Efflux/Figures/figure10_ALLhatchdata.tiff",res=600,width=8,height=6,units="in")
grid::grid.draw(g)
dev.off()


hatch.plot4 <- ggplot(yearly_mean_succ,aes(x=zone,y=mean.success,fill=zone))+
  geom_col(stat = "identity")+
  geom_point(aes(x=zone,y=mean.eff*20),shape=15,size=3,color="midnightblue")+
  geom_errorbar(aes(ymin=lower*20,ymax=upper*20))+
  geom_point(aes(y=tre.zone*20),color="grey23",shape=15,size=3)+
  #geom_line(data=total.data.means,aes(x=zone,y=mean.eff*100),group=1)+
  #stat_summary(fun = function (x) sum(x) ,geom="bar")+
  #stat_summary(data=total.data.means,fun.y=mean,fun.ymin = function(x) mean(x)-sd(x)/length(x),fun.ymax = function(x) 
    #mean(x) + sd(x)/length(x),geom="errorbar",color="black")+
  facet_grid(.~location)+
  scale_fill_manual(name="Beach Zone",labels=c("Littoral","Berm","Dunes"),values=c("darkslategray4","darkseagreen3","burlywood3"))+
...
g <- ggplot_gtable(ggplot_build(hatch.plot4))
striprt <- which(grepl('strip-t', g$layout$name))
fills <- c("steelblue3","coral","darkolivegreen4")
k <- 1
for (i in striprt) {
